...
public class RMIClientImpl implements RMIClient {
    ...
    public User getCurrentUser() throws RemoteException {
        return musicService.getUserById(userId);
    }
    ...
    @Override
    public List<Song> getPlayHistory() throws RemoteException {
        List<PlayHistory> PlayHistory = musicService.getHistory(userId);
        List<Song> songs = new ArrayList<>();
        for (PlayHistory playHistory : PlayHistory) {
            songs.add(playHistory.getSong());
        }
        return songs;
    }
    ...
    public List<Song> getSongsByName(String name) throws RemoteException {
        List<Song> songs = musicService.getSongsByName(name);
        return songs;
    }
    ...
    public Artist getArtistById(int artistId) throws RemoteException {
        return musicService.getArtistById(artistId);
    }
    
    @Override
    public List<Artist> getArtistsByName(String name) throws RemoteException {
        return musicService.getArtistsByName(name);
    }
    ...
    public void addArtist(Artist artist) throws RemoteException {
        // For now, we'll just print the artist info since there's no direct method in MusicService
        System.out.println("Added artist: " + artist.getName());
    }

    @Override
    public void addAlbum(Album album) throws RemoteException {
        // Use the insertAlbum method from MusicService
    ...
    public Image getImage(String path) throws RemoteException {
        ...
        try {
            ...
            Image image = ImageConverter.bytesToImage(imageData);
            return image;
        }
        catch (Exception e) {
